...
class MedicineRepository: ObservableObject {
    ...
    func updateMedicine(_ medicine: Medicine, user: String){
        ...
        do {
            ...
            addHistory(action: "Updated \(medicine.name)", user: user, medicineId: medicine.id ?? "Unknow", details: "Updated medicine details")
     ...
     func updateStock(_ medicine: Medicine, by amount: Int, user: String) {
           ...
           db.collection("medicines").document(id).updateData([
               ...
               if let error = error {
                   ...
                   self.addHistory(action: "\(amount > 0 ? "Increased" : "Decreased") stock of \(medicine.name) by \(amount)", user: user, medicineId: medicine.id ?? "Unknow", details: "Stock changed from \(medicine.stock - amount) to \(newStock)")
               }
           }
       }
