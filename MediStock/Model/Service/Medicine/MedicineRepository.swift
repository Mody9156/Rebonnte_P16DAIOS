...
class MedicineRepository: ObservableObject {
    ...
    func setDataToAisle() async throws -> [Medicine] {
        do{
            return try await medicineService.setDataToAisle()
        }catch{
            throw MedicineError.invalidSetData
        }
    }
    ...
    func updateStock(_ medicine: Medicine, by amount: Int, user: String) {
       let medicine = medicineService.updateStock(medicine, by: amount, user: user)
        ...
        for medicines in medicine {
            ...
            addHistory(action: "\(amount > 0 ? "Increased" : "Decreased") stock of \(medicines.name) by \(amount)", user: self.identity, medicineId: medicines.id ?? "Unknow", details: "Stock changed from \(medicines.stock) to \(newStock)")
        }
    }
